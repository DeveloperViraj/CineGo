...
const SeatLayout = () => {
  ...
  const getoccupiedSeats = async () => {
    try {
      ...
      if (data.success) {
        // FIXED: convert object to array of seat keys
          ...
          return (
            <button
              ...
              className={`aspect-square rounded border border-primary/60 text-xs md:text-base transition
                ...
                ${isOccupied ? 'opacity-30 bg-gray-400 cursor-not-allowed' : ''}`}
              title={seatId}
            >
              {seatId}
            </button>
        ...
        <div className='mt-4 space-y-1'>
          {show?.datetime?.[date]?.map((item) => (
            <div
              ...
              className={`flex items-center gap-2 px-10 py-2 w-max rounded-r-md cursor-pointer transition
      ...
      <div className='relative flex flex-1 flex-col items-center max-md:mt-15'>
        ...
        <div className='flex justify-center mt-10 max-md:mt-0'>
          <button
            ...
            className='flex gap-2 px-8 py-3 text-md bg-primary hover:bg-primary-dull rounded-full transtion font-medium cursor-pointer max-md:px-5 max-md:text-sm my-5 max-md:pb-2'
          >
            Proceed to checkout
