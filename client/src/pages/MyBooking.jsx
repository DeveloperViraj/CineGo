        ...
        {bookings.map((data, index) => {if (!data.show?.movie) return null;
          ...
          >
            ...
            <div className="text-white text-sm flex flex-col gap-1 px-1">
              ...
              {data.isPaid ? (
  <span className="px-4 py-1.5 mb-2 text-sm bg-green-600/90 rounded-full font-medium cursor-default">
    ✅ Successfully Booked
  </span>
) : (
  <Link
    to={data.paymentLink}
    className="px-4 py-1.5 mb-2 text-sm bg-primary hover:bg-primary-dull rounded-full transition font-medium cursor-pointer"
  >
    Pay Now
  </Link>
)}
