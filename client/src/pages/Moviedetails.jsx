...
const Moviedetails = () => {
  const { shows, getToken, user, favorites, bumpFavorites } = useAppContext();
  const { id } = useParams();
  ...
  const handleDate = () => toast('Please choose a date');
  ...
  const isFaved = favorites?.some((m) => m?._id === id);
        ...
        <div className="text-white relative flex flex-col gap-4 max-w-xl">
          ...
          <div className="flex flex-wrap gap-4 mt-4 max-md:gap-2">
            {movie.trailer ? (
              ...
              <button
                disabled
                className="flex items-center px-5 py-2 bg-gray-600 cursor-not-allowed text-gray-400 rounded-lg text-sm font-medium max-md:px-3"
              >
                <PlayCircleIcon className="w-5 h-5 mr-2" />
                No Trailer
              </button>
            ...
            >
              <Heart className={`w-5 h-5 ${isFaved ? 'text-primary fill-primary' : ''}`} />
            </button>
          </div>
        </div>
      </div>
    ...
    {Array.isArray(movie.casts) && movie.casts.slice(0, 12).map((cast, index) => {
      ...
      <div className='px-6 md:px-8 lg:px-16 xl:px-20 overflow-hidden py-10 max-md:py-0'>
        ...
        <div className='flex flex-wrap max-sm:justify-center gap-8 mt-8'>
          {shows?.filter((s) => s && s._id).slice(0, 4).map((show) => (
